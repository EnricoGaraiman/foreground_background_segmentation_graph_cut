'''
===============================================================================
   Draw a rectangle around the object using
...
class GraphCut:
    def __init__(self, img, mask, rect=None, gmm_components=5, save_graph=False):
    ...
    def construct_gc_graph(self):
        ...
        if self.save_graph:
            edges = [list(ele) for ele in edges]

            # plot graph
            node_colors = ['red' if i in bgd_indexes[0] else 'green' if i in fgd_indexes[0] else 'gray' for i in
                           range(self.cols * self.rows)]

            g = ig.Graph(edges=edges)
            ig.plot(g,
                    target=r'E:\1. Documente Facultate\_Master SIVA Anul 1, SEM 2\CV2\foreground_background_segmentation_graph_cut\results\graph.pdf',
                    vertex_color=node_colors)

            # fig, ax = plt.subplots()
            # ig.plot(g, target=ax, vertex_color=node_colors)
            # plt.show()
            print('Save graph')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        ...
        filename = r'E:\1. Documente Facultate\_Master SIVA Anul 1, SEM 2\CV2\foreground_background_segmentation_graph_cut\resource\women.jpg'
    ...
    while (1):
        ...
        if k == 27:  # esc to exit
            ...
            cv.imwrite('graphcut_output.png', res)
            ...
            and again press 'n' \n""")
            print(rect)
            if (rect_or_mask == 0):  # graphcut with rect
                gc = GraphCut(img2, mask, rect)
                rect_or_mask = 1
            elif rect_or_mask == 1:  # graphcut with mask
